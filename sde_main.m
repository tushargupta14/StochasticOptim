...
while iteration < N
    ...
    for t = 1:1:n-1
        t_curr = t;
        t_next = t+1;
        Cs = 6.29 * 10^-2 + 2.46*10^-3 * (T_vec(1,n-t_curr+1)-273) - 7.14 * 10^-6 * (T_vec(1,n-t_curr+1)-273)^2 ;%%%%
        S  = (y_mat(n-t_curr+1,1) - Cs)/Cs; %%%%%%
        G = (kg* exp(-E_g/T_vec(1,n-t_curr+1)))*S^g; %%%%%
        z_mat(t_next,:) = z_backward(z_mat(t_curr,:),y_mat(n-t_curr+1,:),G,T_vec(1,n-t_curr+1),S,t_curr,t_next);  %%%%%
    end
    ...
    for t = 1:1:n-1
        ...
        S  = (y_mat(n-t_curr+1,1) - Cs)/Cs; %%%%%%
        G = (kg* exp(-E_g/T_vec(1,n-t_curr+1)))*S^g; %%%%%
        w_mat(t_next,:) = w_backward(w_mat(t_curr,:),z_mat(t_curr,:),y_mat(n-t_curr+1,:),G,T_vec(1,n-t_curr+1),S,t_curr,t_next);  %%%%%
    end
    ...
    for t = 1:1:n-1
        ...
        fi_mat(t_next,:) = fi_backward(fi_mat(t_curr,:),delta_t,G,y_mat(n-t_curr+1,:),z_mat(t_curr,:),theta_mat(n-t_curr+1,:),T_vec(1,n-t_curr+1),t_curr,t_next);
    ...
    for t = 1:1:n-1
        ...
        psi_mat(t_next,:) = psi_backward(psi_mat(t_curr,:),fi_mat(t_curr,:),delta_t,G,w_mat(t_curr,:),y_mat(n-t_curr+1,:),z_mat(t_curr,:),theta_mat(n-t_curr+1,:),T_vec(1,n-t_curr+1),t_curr,t_next);
    ...
    for t = 1:1:n
        sum = 0;
        for i=1:1:9
            sum = sum + DelH_dy_mat(t,i)*theta_mat(t,i) + DelH_dz_mat(t,i)*fi_mat(t,i)+DelH_dw(i)*psi_mat(t,i);    %%$$$$$$$$$$$
            %theta_mat(t,i)
        end
